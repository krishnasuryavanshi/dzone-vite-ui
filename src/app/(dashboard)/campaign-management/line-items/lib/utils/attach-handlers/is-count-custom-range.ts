...
export const isCountCustomRange = (patchFormValues: Function, updateFormStepDetails: Function) => {
  const onChange = (e: any) => {
    patchFormValues({
      [LineItemFields.IsCompanySizeEmployeeCountCustom]: e.target.value,
    });

    updateCountCustomRangeField(e.target.value, patchFormValues, updateFormStepDetails);
  };
