...
export const getAnalyticsJobTitleData = () => {
  ...
  const aiSuggestedAcceptedJobtitles = analyticsJobTitlesData?.jtList
    ?.filter(({ type }: Record<string, any>) => type === JobTitleTokenType.AIRecommended)
    ?.map(({ text }: Record<string, any>) => text);

  const jobTitleRecommendationAnalytics = (analyticsJobTitlesData?.recommendedJobTitles || []).map(
    ({ name: originalJobTitle, children }: Record<string, any>) => {
      const suggestedJobTitles = children.map(({ label }: Record<string, string>) => {
        return label;
      });
      const acceptedJobTitles: string[] = aiSuggestedAcceptedJobtitles.filter((jobTitle: string) =>
        suggestedJobTitles.includes(jobTitle),
      );
