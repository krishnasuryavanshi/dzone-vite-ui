...
export const prepareViewData = (data: ICampaign, options?: Record<string, any>) => {
  return {
    ...data,
    [CampaignField.IoNumber]: options?.restrictedFields?.includes(CampaignField.IoNumber)
      ? null
      : data?.ioNumber,
    [CampaignField.BookedRevenue]: options?.restrictedFields?.includes(CampaignField.BookedRevenue)
      ? null
      : data?.bookedRevenue,
    ...
    campaignDuration: calculateDateDiffs(data?.targetStartDate, data?.targetEndDate),
