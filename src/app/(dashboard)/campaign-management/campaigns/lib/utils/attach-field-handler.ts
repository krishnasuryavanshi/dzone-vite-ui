import { extractFieldNamesForValidation, fieldValidationHandler } from '../../../lib/utils';
...
export const attachFieldHandlers = (
  ...
  switch (step) {
    case CampaignStep.BasicInfo:
      updateFormStepDetails(CampaignFormSection.MarketerDetails, CampaignField.MarketerCode, {
      ...
      if (isEditing) {
        updateFormStepDetails(CampaignFormSection.CampaignDetails, CampaignField.Status, {
          onChange: (val: string, option: Record<string, string>) => {
            patchFormValues({
    ...
    case CampaignStep.Campaign:
      ...
      for (const field of Object.values(requiredFieldsForValidationCampaignStep)) {
        fieldValidationHandler(
          patchFormValues,
          updateFormStepDetails,
