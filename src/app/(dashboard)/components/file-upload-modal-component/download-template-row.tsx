...
export const DownloadTemplateRow: FC<IDownloadTemplateRowProps> = ({
  ...
  const handleDownloadtemplate = async () => {
    try {
      setIsLoading(true);
      const { data, headers } = await downloadDataMapperFileTemplate(downloadTemplateProps?.type);
      if (data) {
  ...
  return (
    <Flex justify='center' gap={'1rem'} align='center'>
      ...
      <HasPermission
        permissions={[DeliveryTemplateActionsEnum.Create, DeliveryTemplateActionsEnum.Edit]}
      >
        <Button
          type='primary'
          size='small'
        ...
        >
          {isLoading ? <LoadingOutlined /> : <Translate i18nKey='Download Template' />}
        </Button>
      </HasPermission>
    </Flex>
  );
